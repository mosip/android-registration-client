...
class _CustomDynamicDropDownState extends State<GenderControl> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      if (context
        ...
        // Re-check: if OCR or external source wrote to fieldInputValue during the await,
        // read that value instead of overwriting with the server default.
  ...
  Future<void> _getSelectedValueFromMap(String lang) async {
    ...
    fieldValueData = data;
    String response = data.isNotEmpty ? (data[0]?.name ?? "") : "";
    final raw = globalProvider.fieldInputValue[widget.field.id ?? ""];
    if (widget.field.type == 'simpleType') {
      if (raw is Map && raw.containsKey(lang)) {
        response = raw[lang]?.toString() ?? response;
      } else if (raw is Map && raw.isNotEmpty) {
        response = raw.values.first?.toString() ?? response;
      }
    } else if (raw != null) {
    ...
    if (fieldValueData != null) {
      for (var e in fieldValueData!) {
        if (e != null &&
            (e.name.toLowerCase() == response.toLowerCase() ||
             e.code.toLowerCase() == response.toLowerCase())) {
  ...
  Widget build(BuildContext context) {
    return Consumer<GlobalProvider>(
      builder: (context, _, __) {
        ...
        // Re-sync selected value from fieldInputValue (handles external updates like OCR)
        if (globalProvider.fieldInputValue
            .containsKey(widget.field.id ?? "")) {
          String mapValue = "";
          final raw = globalProvider.fieldInputValue[widget.field.id ?? ""];
          if (widget.field.type == 'simpleType') {
            if (raw is Map && raw.containsKey(mandatoryLangCode)) {
              mapValue = raw[mandatoryLangCode]?.toString() ?? "";
            } else if (raw is Map && raw.isNotEmpty) {
              mapValue = raw.values.first?.toString() ?? "";
            } else {
              mapValue = selected ?? "";
            }
          } else {
            mapValue = raw?.toString() ?? "";
          }
          if (mapValue.isNotEmpty && mapValue.toLowerCase() != selected?.toLowerCase()) {
            selected = mapValue;
            if (fieldValueData != null) {
              for (var e in fieldValueData!) {
                if (e != null &&
                    (e.name.toLowerCase() == mapValue.toLowerCase() ||
                     e.code.toLowerCase() == mapValue.toLowerCase())) {
                  saveData(e.name, e.code);
                  break;
                }
              }
