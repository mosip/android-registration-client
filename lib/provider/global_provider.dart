...
class GlobalProvider with ChangeNotifier {
  ...
  /// Refresh all configuration-dependent data after configuration changes
  /// This method refreshes data that depends on global parameters like:
  /// - getCachedStringAgeGroup
  /// - getCachedStringMAVELScript
  /// - getCachedStringPreRegPacketLocation
  /// - And other configuration-dependent data
  Future<void> refreshConfigurationDependentData() async {
    try {
      log("Starting configuration-dependent data refresh...");
      
      // Refresh age group configuration
      await _refreshAgeGroupData();
      
      // Refresh MAVEL script configuration
      await _refreshMAVELScriptData();
      
      // Refresh pre-registration packet location
      await _refreshPreRegPacketLocationData();
      
      // Refresh other configuration-dependent data
      await _refreshOtherConfigurationData();
      
      log("Configuration-dependent data refresh completed successfully");
    } catch (e) {
      log("Error refreshing configuration-dependent data: $e");
    }
  }

  /// Refresh age group data from configuration
  Future<void> _refreshAgeGroupData() async {
    try {
      // This would typically call the Android service to get updated age group config
      // For now, we'll just log that this data should be refreshed
      log("Refreshing age group configuration data...");
      // TODO: Call Android service to refresh age group data
      // String ageGroupConfig = await CommonDetailsApi().getCachedStringAgeGroup();
    } catch (e) {
      log("Error refreshing age group data: $e");
    }
  }

  /// Refresh MAVEL script data from configuration
  Future<void> _refreshMAVELScriptData() async {
    try {
      log("Refreshing MAVEL script configuration data...");
      // TODO: Call Android service to refresh MAVEL script data
      // String mavelScript = await CommonDetailsApi().getCachedStringMAVELScript();
    } catch (e) {
      log("Error refreshing MAVEL script data: $e");
    }
  }

  /// Refresh pre-registration packet location data from configuration
  Future<void> _refreshPreRegPacketLocationData() async {
    try {
      log("Refreshing pre-registration packet location configuration data...");
      // TODO: Call Android service to refresh pre-reg packet location data
      // String preRegPacketLocation = await CommonDetailsApi().getCachedStringPreRegPacketLocation();
    } catch (e) {
      log("Error refreshing pre-reg packet location data: $e");
    }
  }

  /// Refresh other configuration-dependent data
  Future<void> _refreshOtherConfigurationData() async {
    try {
      log("Refreshing other configuration-dependent data...");
      
      // Refresh any other data that depends on global parameters
      // This could include:
      ...
      // For now, we'll just notify listeners to refresh any cached data
      notifyListeners();
    } catch (e) {
      log("Error refreshing other configuration data: $e");
    }
  }
}
