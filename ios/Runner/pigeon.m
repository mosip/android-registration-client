...
void CommonDetailsApiSetup(id<FlutterBinaryMessenger> binaryMessenger, NSObject<CommonDetailsApi> *api) {
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        initWithName:@"dev.flutter.pigeon.registration_client.CommonDetailsApi.saveScreenHeaderToGlobalParam"
        binaryMessenger:binaryMessenger
        codec:CommonDetailsApiGetCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(saveScreenHeaderToGlobalParamId:value:completion:)], @"CommonDetailsApi api (%@) doesn't respond to @selector(saveScreenHeaderToGlobalParamId:value:completion:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        NSString *arg_id = GetNullableObjectAtIndex(args, 0);
        NSString *arg_value = GetNullableObjectAtIndex(args, 1);
        [api saveScreenHeaderToGlobalParamId:arg_id value:arg_value completion:^(NSString *_Nullable output, FlutterError *_Nullable error) {
          callback(wrapResult(output, error));
        }];
      }];
    } else {
