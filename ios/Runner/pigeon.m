  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        ...
        codec:RegistrationDataApiGetCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(evaluateMVELVisibleFieldData:expression:completion:)], @"RegistrationDataApi api (%@) doesn't respond to @selector(evaluateMVELVisibleFieldData:expression:completion:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        NSString *arg_fieldData = GetNullableObjectAtIndex(args, 0);
        NSString *arg_expression = GetNullableObjectAtIndex(args, 1);
        [api evaluateMVELVisibleFieldData:arg_fieldData expression:arg_expression completion:^(NSNumber *_Nullable output, FlutterError *_Nullable error) {
          callback(wrapResult(output, error));
        }];
      }];
    } else {
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        ...
        codec:RegistrationDataApiGetCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(evaluateMVELRequiredFieldData:expression:completion:)], @"RegistrationDataApi api (%@) doesn't respond to @selector(evaluateMVELRequiredFieldData:expression:completion:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        NSString *arg_fieldData = GetNullableObjectAtIndex(args, 0);
        NSString *arg_expression = GetNullableObjectAtIndex(args, 1);
        [api evaluateMVELRequiredFieldData:arg_fieldData expression:arg_expression completion:^(NSNumber *_Nullable output, FlutterError *_Nullable error) {
          callback(wrapResult(output, error));
        }];
      }];
    } else {
