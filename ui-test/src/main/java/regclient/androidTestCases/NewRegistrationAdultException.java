...
public class NewRegistrationAdultException extends AndroidBaseTest {
	...
	@Test(priority = 0, description = "Verify adult new registration exception")
	public void newRegistrationAdultException() throws InterruptedException {
		...
		for (String screen : screenOrder) {
			if (screen.equals("consentdet") || screen.equals("consent")) {
				...
				documentuploadPage.uploadDoccuments("adult", "withoutReferenceNumber");
				...
				assertTrue(biometricDetailsPage.isBiometricDetailsPageDisplayed(),
						"Verify if user is not navigated to the next page without biometric scan");
				if ("eng".equalsIgnoreCase(language)) {
				...
				// Exception
				assertTrue(biometricDetailsPage.isBiometricDetailsPageDisplayed(),
						"Verify if biometric details page is still displayed without capture exception");
...
//		assertTrue(previewPage.isBiometricsInformationInPreviewPagePageDisplayed(),"Verify if Biometrics Information In PreviewPage is displayed");
		// assertTrue(previewPage.isSingleIrisImageDisplayed(),"Verify if single iris
		// exception image In PreviewPage is displayed");
