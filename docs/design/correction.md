## Correction Flows (Biometric & Demographic)
...
Note: The exact on‑screen wording and where this text appears in the app depend on the UI specification.
...
Note: This document centers on the Biometric Correction flow, but the same high‑level flow, validation, persistence and offline handling can be reused for Demographic Correction (for example: name, date of birth, address, document fields) where the UI/spec enables it. Demographic correction must follow the same security, re‑authentication, preview and audit requirements; adapt validations and PII handling to the data type as required by policy.
...
- Operations:
    ...
    - Applicant Demographic Correction (e.g., name, DOB, address, documents) — UI dependent; when enabled, follow the same flow boundaries (preview, re‑auth, submission, audit, offline queue).
...
- Capture uses the platform SPI/SDK in the following sequence:
    1. Check device connectivity and permissions.
...
- Thresholds per modality, max retries
...
- Demographic correction: where the UI/spec supports editing demographic fields, the same preview, re‑auth and offline queue behavior must apply; enforce PII validation and audit trail similarly to biometric correction.
...
![CorrectionFlow.png](../CorrectionFlow.png)
