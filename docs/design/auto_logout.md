...
- Configuration properties:
    - `idleDuration`: `mosip.registration.idle_time`.
    - `graceDuration`: `mosip.registration.refreshed_login_time`.
...
- Typical values:
    ...
    - Grace period: e.g., 5 minutes,  During grace period, a warning UI can be shown (implementation-specific).
...
- `lib/utils/inactivity_tracker.dart`: Utility/widget that implements the inactivity tracking logic.

## Sequence Diagram
The following sequence diagram illustrates the registration process, including the use of the Auto logout Feature:

![AutoLogoutFlow.png](../AutoLogoutFlow.png)
